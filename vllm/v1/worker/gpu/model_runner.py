    ...
    def sample_tokens(
        ...
        if self.num_speculative_steps > 0:
            ...
            if self.scheduler_config.async_scheduling:
                # The scheduler has already placeholder-scheduled the next step, so
                # truncating here would starve positions it committed to. Report the
                # decision instead; the scheduler applies it when shaping the step
                # after next (one extra step of staleness, which the controller's
                # two-in-a-row entry and sticky exit already tolerate).
                model_runner_output.next_num_spec_tokens = num_draft
                num_draft = None
