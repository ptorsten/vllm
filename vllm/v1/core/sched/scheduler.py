...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # Latest verify-block length requested by the worker's speculator
        # (DFlash2 lookup adaptive controller). Arrives via ModelRunnerOutput,
        # i.e. after the next step was already scheduled under async
        # scheduling, so it takes effect one schedule later.
        self.worker_num_spec_tokens: int | None = None
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        elif self.worker_num_spec_tokens is not None:
            num_spec_tokens_to_schedule = min(
                max(self.worker_num_spec_tokens, 1), self.num_spec_tokens
            )
    ...
    def update_from_output(
        ...
        # Verify-block length requested by the speculator's adaptive controller
        # (getattr: an EMPTY_MODEL_RUNNER_OUTPUT copy predating the field, or a
        # worker without the overlay, must read as "no request").
        self.worker_num_spec_tokens = getattr(
            model_runner_output, "next_num_spec_tokens", None
        )
